...
// 		$data .= ", type = '$type' ";
	...
	function save_check_in(){
		...
		// COUNT 
		$query  = $this->db->query("SELECT count(1) FROM reserved");
		...
		$fName = strtoupper(substr("$name", 0, 2));
		$transDate = strtoupper(date('My'));
		$date_trans = date("Y-m-d");
		$bookDate = date('my');
		$data .= ", date_trans = '$date_trans' ";

		// REFERENCE NO. 
		$ref = $fName . $transDate . $bookDate . '-' . $roomType . $count;


		$i = 1;
		while($i== 1){
			if($this->db->query("SELECT * FROM reserved where ref_no ='$ref'")->num_rows <= 0)
				$i=0;
		}
		$data .= ", ref_no = '$ref' ";
	...
	function save_checkout(){
		extract($_POST);
			$save = $this->db->query("UPDATE reserved set status = 2 where id=".$id);
			if($save){
