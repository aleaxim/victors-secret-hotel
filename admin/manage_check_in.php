...
<div class="container-fluid">
	...
	<form action="" id="manage-check">
		...
		<!-- If Room is available or matches with room id  -->
		...
		<div class="form-group">
			...
			<select name="rid" id="" class="custom-select browser-default">		
				<!-- From the available rooms, get its rid  -->
				<?php while($row=$rooms->fetch_assoc()): ?>
				<option value="<?php echo $row['id'] ?>"<?php echo $row['id'] == $rid ? "selected": '' ?>><?php echo $row['room'] . " | ". ($cat_arr[$row['category_id']]['name']) ?></option>
					<?php $room_cat = $cat_arr[$row['category_id']]['name']; ?>
					<?php $cid = $row['category_id']; ?> 
				<?php endwhile; ?>
			</select>		
		</div>
		<!-- Save Room Category and Category ID -->
		<?php $_GET['room_cat'] = $room_cat;?>
		<input type="hidden" name="room_cat" value="<?php echo isset($_GET['room_cat']) ? $_GET['room_cat']: '' ?>">
		<?php $_GET['cid'] = $cid;?>
